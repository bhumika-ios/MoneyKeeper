...
extension MoneySpent {
    ...
    static func fetchAllCategoriesTotalAmountSum(context: NSManagedObjectContext, completion: @escaping ([(sum: Double, category: Category)]) -> ()) {
        let keypathAmount = NSExpression(forKeyPath: \MoneySpent.amount)
        ...
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: MoneySpent.entity().name ?? "MoneySpent")
